...
window.processOAuthCode = function processOAuthCode(data) {
  alert("BERNARD processOAuthCode");
...
window.onOAuthComplete = function onOAuthComplete(token,OAuthSchemeKey) {
  console.log("XXXXXXXXXXXXXXXXXXXXXXXXXXX BERNARD onOAuthComplete");
  console.log(token);
  var output = '';
  for (var p in token) {
    output = output + " "+p +" "+token[p];
  }
console.log(output);
  ...
  if(token) {
    if(token.error) {
      ...
      console.log("BERNARD"+window.swaggerUi.tokenName);
      if (!OAuthSchemeKey){
          AuthSchemeKey = token.state;
      }
      console.log("BERNARD1  "+OAuthSchemeKey);
      console.log("BERNARD2  "+b);
      if(b){
        // if all roles are satisfied
        var o = null;
        console.log("BERNARD4 HERE");
	...
	console.log("BERN HERE "+OAuthSchemeKey+"   "+b);
